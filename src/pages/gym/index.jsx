...
const Index = () => {
  ...
  const getFormalCart = () => {
    if (screenWidth <= 767) return gym.slice(0, 6);
    if (screenWidth <= 1024) return gym.slice(0, 8);
    if (screenWidth <= 1280) return gym.slice(0, 6);
    if (screenWidth >= 1281) return gym.slice(0, 8);
    return gym;
  };
  const gymProducts = getFormalCart()
  const openFilter = () => {
        ...
        <div className="hidden lg:block mb-5 w-64 2xl:w-96 border border-black p-4 rounded-lg shadow-md space-y-6">
          ...
          <div className="space-y-2 2xl:space-y-1">
            ...
            <div>
              ...
              <p className="cursor-pointer hover:bg-slate-200 rounded-md px-2 2xl:text-xl">Gym</p>
            </div>
          </div>
